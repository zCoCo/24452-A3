...
function rot_q14()
    ...
    function xx = x_r(t,row) % for row in fourier series table
        xx = 0;
        for j = 1:row
            n = j*2-1; % 1,3,...
            ...
            AR = amp(idx); % Amplitude Ratio at w
            shift = pha(idx); % Phase Shift at w
            xx = xx + AR.*F0.*sin(w.*t.*2.*pi + shift); % sum up each row
        end
    end
