classdef ETable < dynamicprops & matlab.mixin.SetGet
    ...
    methods(Static)
        function ext = cache_extension()
            ext = ".cachedET.mat"; % Unique Extension for Cached ETable files
        end
        
        % Extention of Normal ETable Constructor which saves the output of
        % the constructor as a .mat file with the same name as the file 
        % (including its extension).
        % The second time this function is called on the same file, if the
        % cached .mat file already exists, it will load that file instead.
        % Since the ETable constructor can take a long time to run on
        % massive data files, this saves a lot of time when using the same
        % file a lot.
        % The downside is that if you modify the source file after it's
        % already been cached, this won't pick up that change unless the
        % you delete the .mat cache file.
        %
        % If 'recache' is true, existing cache file will be ignored and
        % the ETable will be reconstructed and cached.
        %
        % This takes the same inputs as the normal ETable constructor
        % EXCEPT that 'source' must be a path to a file.
        function obj = CachedLoad(source, options, recache)
            if nargin < 2
                options = struct();
            end
            if nargin < 3
            ...
            ext = ETable.cache_extension();
            load_from_cache = ~recache && isfile(char(source+ext)); % Whether to load file directly from cache
            if load_from_cache
                try
                    % Load cached file if there is one
                    load(char(source+ext), 'obj');
                catch e
                    % If there's an issue loading from cache, just load the
                    % source file instead.
                    warning(e.message);
            ...
            if ~load_from_cache
                ...
                % cache it.
                obj = ETable(char(source), options);
                save(char(source+ext), 'obj');
            end
        end
        
        % Performs #cachedLoad on all files in the given directory,
        % applying the given options to each file.
        % Since the caching process can take a while on the first run, this
        % allows the entire process to be batched in case there are a lot
        % of files.
        %
        % If 'recache' is true, all existing cache files will be ignored 
        % and ETables will be reconstructed and cached for every file.
        %
        % If 'deep' is set to true, all files in all subdirectories (and 
        % the subdirectories of all subdirectories, etc.) will be included.
        %
        % If 'allowedExtensions' is given, this will only cache files with the
        % given list of extensions. The default is {"xlsx", "txt", "csv"}.
        %
        % If 'mustInclude' is given, only files whose names include the
        % given string are selected (can also be a vector of strings).
        % If 'mustExclude is given, files whose names include the given
        % string are excluded (can also be a vector of strings).
        function CacheDirectory(directory, options, recache, deep, allowedExtensions, mustInclude, mustExclude)
            if nargin < 5
                allowedExtensions = ["xlsx", "txt", "csv"];
            else
            ...
            % Cache all files in given directory:
            function cacheDir(directory)
                for ext = allowedExtensions
                    % Grab all files with given extension:
                    files = dir(fullfile(directory,"*."+ext));
                    % And with appropriate names:
                    files = files(contains({files.name}, mustInclude));
                    if mustExclude ~= ""
                        files = files(~contains({files.name}, mustExclude));
                    end
                    % Ignore OS hidden files:
                    files = files(~startsWith({files.name}, '._'));
                    
                    % Attempt to cache each file:
                    for file = string({files.name})
                        filename = fullfile(directory,file);
                        disp("Caching " + filename + ". . .");
                        try
                            ETable.CachedLoad(filename, options, recache);
                        catch e
                            disp("> Caching Failed.");
                            warning(e.message);
                        end
            ...
            % Use helper function to recurse through all subdirectories:
            function cacheDirDeep(directory)
                disp("In Directory " + directory + ":");
                % Cache all files in given directory:
                cacheDir(directory);
                % Find all subdirectories and cache there:
                directories = dir(directory); % Grab everything in directory
                directories = directories([directories.isdir]); % Select only subdirectories
                % Ignore OS hidden directories and path shortcuts (. and
                % ..):
                directories = directories(~startsWith({directories.name}, '.'));
                % Cache subdirectories:
                for subdir = string({directories.name})
                    cacheDirDeep(fullfile(directory,subdir));
                end
            end
            
            if(nargin > 3 && deep)
                cacheDirDeep(directory);
            else
                cacheDir(directory); % Just cache files in the given directory
            end
        end
    end
    ...
    methods
        ...
        % Arguments:
        % 'source':
        % Can be a Table, ETable, or the address/path to a
        % spreadsheet file (.xlsx, .txt, .csv are supported).
        % 'options':
        % - 'shortNames': List of Shorthand Names for Each 
        % Column in the Given Table. Must have same number of entries as 
        % columns in the Table. Used to speed up access to these columns. 
        % For example: A file with columns 'time', 'Temperature1', and 
        % 'Temperature2' in the file might be given shortnames
        % ["t","T1","T2"]. These would be accessed as vectors from an 
        % ETable named 'tab' as tab.t, tab.T1, tab.T2.
        % MUST BE a vector of STRINGs (not chars) or a CELL Array of
        % strings or chars.
        %
        % - 'unitsList': Cosmetically Styled Units for Each Short Name 
        % (using latex).
        % MUST BE a vector of STRINGs (not chars) or a CELL Array of
        % strings or chars.
        %
        % - 'isText' : Whether the given file is text-based.
        %
        % - 'delimiter': The delimiter for text or csv files.
        %
        % - 'ignoreLines': specifies how many lines of 
        % meta-data that do not contribute to a file's actual table there
        % are. These will be ignored. This only applies to text-based files
        % such as .txt or .csv.
        function obj = ETable(source, options)
            % Creates a New ETable with the same contents as the given ETable
            % if only one argument is given and that argument is the 
            % ETable
            
            % Use Default Values for Options not Given:
            % If a option should just be ignored if it is not given (as in
            % having a default value doesn't make sense for that value, 
            % eg. 'unitsList'), it is not included.
            defaults = struct(...
                'isText', false, ...
                'delimiter', '\t', ...
                'ignoreLines', 0 ...
            ...
            if nargin > 1
                ...
                fieldsNotGiven = defaultFields(~ismember(fieldnames(defaults), fieldnames(options)));
                for i = 1:numel(fieldsNotGiven)
                    % Add default value to options for all fields not given:
                    options.(fieldsNotGiven{i}) = defaults.(fieldsNotGiven{i});
                end
            else
                options = defaults;
            end
            
            if isa(source, 'ETable')
                other = source; 
                obj.data = other.data;
                ...
                if istable(source)
                    ...
                    if options.isText
                        % Special Text-File Input:
                        opts = delimitedTextImportOptions('DataLines',options.ignoreLines, 'Delimiter', options.delimiter);
                        obj.data = readtable(source, opts, 'ReadVariableNames',false);
                    else
                        % Default Input:
                ...
                % If No shortNames were given, use Table VariableNames:
                if isfield(options, 'shortNames')
                    obj.shortNames = string(options.shortNames);
                else
                    obj.shortNames = string(obj.data.Properties.VariableNames);
                end
                ...
                for c = 1:width(obj.data)
                    ...
                    try
                        obj.addprop(char(obj.shortNames(c)));
                        if numericData
                            tabData = cell2mat(obj.data{:,c});
                        else
                            tabData = string(obj.data{:,c});
                        end
                        obj.set(char(obj.shortNames(c)), tabData);
                    catch e
                        warning(e.message);
            ...
            if isfield(options, 'unitsList')
                obj.unitsList = string(options.unitsList);
            else
