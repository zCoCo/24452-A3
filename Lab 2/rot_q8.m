...
%% STEP RESPONSE TESTS:
    F0 = 0.25; % [N] Impulse amplitude
    w0 = 2*pi*2; % [rad/s] Impulse Freq.
    t0 = 0; % Start time
    u = @(t) F0 * sin(w0*(t-t0)) * (t>t0); % [N] Step Inputu = @(t) F0 * sin(w0*(t-t0)) * (t>t0) * ((t-t0)<0.05); % [N] Step Input (4N delayed by 1s)
    q0 = [0 0]; % [m, m/s] Initial Conditions (x0, v0)
    ms = [M4 M4 M2 M2];
    ks = [1.125 2.25 0.375 1.125] + kphys; % Stiffnesses [N/m] (simulated parallel to physical)
    cs = [0.02 0.02 0.02 0.35] + cphys; % Damping Ratios [kg/s] (simulated in series with physical)
    ...
    for i = 1:4
        ...
        T = load_data(fullfile(root,"2.4","test"+i+".txt"), xunits);
        ...
        saveas(gcf, char("rot_q8-"+i+".png"), 'png');
    ...
    writetable(step_data, 'rot_q8_stepData.xlsx');
    ...
    writetable(step_headers, 'rot_q8_stepDataHeaders.xlsx');
end
