...
%% STEP RESPONSE TESTS:
    u = @(t) 0.25 * (t>1); % [N] Step Input (4N delayed by 1s)
    q0 = [0 0]; % [m, m/s] Initial Conditions (x0, v0)
    ms = [M2 M2 M4 M4 M1 M1 M2 M2];
    ks = [1.125 1.125 1.125 1.875 0.375 3 3.75 1.125] + kphys; % Stiffnesses [N/m] (simulated parallel to physical)
    cs = [0.01 0.005 0.05 0.01 0.15 0.15 0.15 0.45] + cphys; % Damping Ratios [kg/s] (simulated in series with physical)
    ...
    for i = 1:8
        ...
        if(i==1)
            ...
            c = cs(i);
            k = ks(i);
            out = sim('rot_q6_sim', [0 T.t(end)], simset('SrcWorkspace','current'));
            hold on
        ...
        saveas(gcf, char("rot_q6_step-"+i+".png"), 'png');
    ...
    writetable(step_data, 'rot_q6_stepData.xlsx');
    ...
    writetable(step_headers, 'rot_q6_stepDataHeaders.xlsx');
end
